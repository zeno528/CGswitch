...
### 新增
- Skill 支持从本地文件夹导入：导入页新增「从文件夹导入」直接选择目录，缺 SKILL.md 的目录会被拒绝；内容与已管理正本一致时自动跳过；导入完成立即重扫更新角标。
- 添加账号改为卡片式 OAuth 弹窗：登录方式与账号管理特性一目了然，等待授权、重新打开浏览器、取消登录都在弹窗内完成，关闭弹窗即取消登录。
- 系统托盘菜单新增「账号」项：点击显示主窗口并直接打开账号页。
- 配置编辑器新增缩进参考线：按文档实际缩进单位（2/4 空格或 Tab）绘制列对齐参考线，空行继承上下文层级，内容变化时自动重配。

### 修复
- 4K、150% 缩放下页面进场动画末段文字发虚抖动：动画沿原曲线逐帧对齐整设备像素，末段整像素自然落零，可见运动约 330ms（原 700ms 的后半段本就不可见）。
- 编辑器打开长文档时，横向滚动条按整篇最长行在首屏即正确出现（虚拟化下预设全文宽度）。

### 界面与样式
- 编辑器格式化按钮图标统一为代码图标，不再随当前文件类型切换。
- 长上下文开关与紧凑上限输入合并为一组 ghost 控件：悬停整组亮底，组内加分隔线。
- models.json 草稿清空不再弹确认框（auth 清空仍需确认）。
- 账号页额度重置条目底色与主面板对齐。
- Skill 内容预览弹窗加宽，长描述与长行更易读。

### 移除
- 新建 OpenCode 供应商不再显示订阅推广提示。
