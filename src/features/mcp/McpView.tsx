...
export default function McpView() {
  ...
  const removeServer = async (server: McpServerSpec) => {
    ...
    if (!confirmed) return;
    try { await api.deleteMcpServer(server.name); feedback.success("MCP 服务器已删除"); await refresh(true); }
