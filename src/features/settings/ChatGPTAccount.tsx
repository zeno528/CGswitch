...
export default function ChatGPTAccount({ initialStatus, balanceCache }: { initialStatus: AuthStatus; balanceCache?: Record<string, ProfileBalanceInfo> }) {
  ...
  if (status.authenticated) return (
    <div className="space-y-4">
      {status.external ? <div className="rounded-[var(--radius-card)] bg-(--input-bg) p-3 shadow-[0_0_0_1px_var(--panel-ring)]"><div className="flex min-w-0 flex-nowrap items-center gap-3"><Monitor className="h-5 w-5 shrink-0 text-accent" strokeWidth={2} /><div className="flex min-w-0 flex-1 items-baseline gap-2 whitespace-nowrap"><span className="setting-title shrink-0">{t("account.followCodex")}</span><span className="shrink-0 text-sm font-semibold" aria-hidden="true">•</span><span className="mono min-w-0 truncate text-sm font-medium">{status.external.login}</span></div></div><AccountQuota source="desktop" accountId={status.external.id} cachedBalance={balanceCache?.[authQuotaCacheKey("desktop", status.external.id)]} /></div> : null}
      {status.accounts.length ? <div className="space-y-2">{status.accounts.map((account) => <div key={account.id} className="rounded-[var(--radius-card)] bg-(--input-bg) p-3 shadow-[0_0_0_1px_var(--panel-ring)]"><div className="flex min-w-0 flex-nowrap items-center gap-3"><KeyRound className="h-5 w-5 shrink-0 text-accent" strokeWidth={2} /><div className="flex min-w-0 flex-1 items-baseline gap-2 whitespace-nowrap"><span className="setting-title shrink-0">{t("account.oauthDeviceLogin")}</span><span className="shrink-0 text-sm font-semibold" aria-hidden="true">•</span><span className="mono min-w-0 truncate text-sm font-medium">{account.login}</span></div><button type="button" className="apple-action-button apple-action-button--compact shrink-0 whitespace-nowrap text-[var(--danger)]" onClick={() => void removeAccount(account.id)}>{t("account.remove")}</button></div><AccountQuota source="oauth" accountId={account.id} cachedBalance={balanceCache?.[authQuotaCacheKey("oauth", account.id)]} /></div>)}</div> : null}
