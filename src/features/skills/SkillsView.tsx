...
export default function SkillsView({ activationEpoch }: { activationEpoch: number }) {
  ...
  if (importing) return <ImportPage candidates={candidates} selectedPaths={selectedPaths} busy={busy} onBack={() => setImporting(false)} onToggle={toggle} onToggleAll={toggleAll} onConfirm={() => void confirmImport()} onImportFolder={() => void importFromFolder()} />;
...
function ImportPage({ candidates, selectedPaths, busy, onBack, onToggle, onToggleAll, onConfirm, onImportFolder }: { candidates: SkillCandidate[]; selectedPaths: string[]; busy: string | null; onBack: () => void; onToggle: (path: string) => void; onToggleAll: () => void; onConfirm: () => void; onImportFolder: () => void }) {
